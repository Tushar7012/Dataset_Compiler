...
_REQUIRED_FIELDS = ("project_id", "name", "base_url", "model")
...
async def create_provider(payload: dict, session: Session = Depends(get_session)):
    ...
    # Every provider is remote — this app always talks to a remote generator/judge
    # endpoint, never a local one. An explicit api_key wins; otherwise fall back to
    # whichever pre-configured credential (HF router vs. Gemini) matches base_url.
