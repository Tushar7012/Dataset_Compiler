...
def _provider(handler) -> OpenAICompatibleProvider:
    ...
    profile = ProviderProfile(name="test", base_url="http://127.0.0.1:9999", model="test-model")
...
async def test_generate_record_dispatches_sft_prompt_completion_to_a_remote_provider():
    provider = _provider(_qa_response)
    plan = TrainingPlan(
    ...
    record = await generate_record(plan=plan, source=_source(), generator=provider, judge=None, spec=spec)
    assert record is not None
