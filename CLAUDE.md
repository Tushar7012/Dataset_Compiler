...
- Gemini/HF credentials are code-complete (`scripts/set_secrets.py`) but not actually seeded on any machine yet — it needs interactive input, so nobody has run it. The AI-suggested-goal feature's real success path (an actual Gemini API call) is verified only via mocked-transport tests until someone runs the script for real.
