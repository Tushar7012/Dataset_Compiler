...
- `validation/judging.py`'s `judge_quality`/`judge_dpo_preference` never thread a `consent` object into `judge.generate(...)` — if a judge provider is ever configured with `endpoint_scope="remote"`, judging will always fail with `RemoteConsentRequiredError`. Found in passing while building the AI-suggested-goal feature; not fixed, unrelated to that work.
